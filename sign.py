...
BATCH_SIZE = 10  # 10 origins x 10 destinations = 100 elements/request (adjust as needed)
GOOGLE_API_BASE = "https://maps.googleapis.com/maps/api/distancematrix/json"
CLIENT_ID = "gme-yourclientid"
...
def sign_url(input_url: str, client_id: str, secret: str) -> str:
    ...
    url_to_sign = f"{url.path}?{query_string}"
    ...
    return f"https://{url.netloc}{url.path}?{query_string}&signature={safe_signature}"
...
def process_batch(batch, output_writer):
    ...
    url = (
        ...
        f"&departure_time=now&units=imperial"
    )
    signed_url = sign_url(url, CLIENT_ID, SIGNING_SECRET)
    ...
    with open(output_filepath, 'w', newline='', encoding='utf-8') as outfile:
        ...
        for record in valid_rows(filepath):
            ...
            if len(batch) == BATCH_SIZE:
                ...
                time.sleep(1)  # Rate limiting: adjust if needed
                batch = []
