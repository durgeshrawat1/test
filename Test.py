...
async def fetch_single_distance(session, rec, api_key, base_url, retry=0):
    ...
    try:
        ...
        async with session.get(url) as response:
            if response.status == 429:
                if retry < MAX_RETRIES:
                    ...
                    logging.warning(f"🕒 429 Too Many Requests — retrying in {wait_time:.1f}s (attempt {retry+1})")
                    await asyncio.sleep(wait_time)
                    return await fetch_single_distance(session, rec, api_key, base_url, retry=retry+1)
                else:
                    logging.error("❌ Max retries reached for rate limiting.")
                    return None

            if response.status != 200:
                logging.error(f"❌ API returned status {response.status} for employee {sanitize(rec['employee'])}")
                return None
        ...
        try:
            ...
            distance, duration = {}, {}
            logging.warning(f"⚠️ Missing data for {sanitize(rec['employee'])}: {sanitize(str(e))}")
...
async def fetch_all_distances(records, api_key, base_url, concurrency=10):
    ...
    async with aiohttp.ClientSession() as session:
        async def safe_fetch(rec):
            async with semaphore:
                return await fetch_single_distance(session, rec, api_key, base_url)
...
async def main():
    ...
    base_url = config['google_api']['base_url']
    ...
    logging.info(f"🚀 Starting fetch for {len(data)} records with concurrency=10 and rate limit=10 elem/sec")

    all_results = await fetch_all_distances(data, api_key, base_url, concurrency=10)
